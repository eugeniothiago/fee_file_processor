...
data_list = os.listdir(f'{project_dir}/fee_data/')

if not data_list:
    print('Nenhum arquivo a ser processado.')
    sys.exit()
else:
    for file in os.listdir(f'{data_list}'):
        if not file:
            print('Não há arquivos processados. O script será encerrado.')
            sys.exit()
...
if file_list:
    ...
    print('Não há dados a serem processados. O script será encerrado.')
    time.sleep(1)
    sys.exit()
